<?php session_start();
    ...
    $get_purchased_datacard_pin_fetch = mysqli_fetch_assoc(mysqli_query($conn_server_db, "SELECT network_name, data_size, card_quality, card_array FROM data_card_history WHERE id='$transaction_ref'"));
...
<?php if(strtolower($transaction_details["transaction_type"]) === "data-card"){ ?>
<div id="receiptDiv" style="margin-top:5%; margin-left:2%; padding:3% 0% 3% 0%; width:96%; background: transparent; height: auto;">
    <?php
        $purchased_datacard_pin_array = array_filter(explode("\n",trim($get_purchased_datacard_pin_fetch["card_array"])));
        
        foreach($purchased_datacard_pin_array as $pins){
            if(strtolower($get_purchased_datacard_pin_fetch["network_name"]) == "mtn"){
                $net_code = "*460*6*1# then enter PIN";
                $data_bal = "*312*5#";
            }

            if(strtolower($get_purchased_datacard_pin_fetch["network_name"]) == "airtel"){
                $net_code = "";
                $data_bal = "";
            }

            if(strtolower($get_purchased_datacard_pin_fetch["network_name"]) == "glo"){
                $net_code = "";
                $data_bal = "";
            }

            if(strtolower($get_purchased_datacard_pin_fetch["network_name"]) == "9mobile"){
                $net_code = "";
                $data_bal = "";
            }
            ...
            <img class="airtime-carrier-logo " src="images/'.strtolower($get_purchased_datacard_pin_fetch["network_name"]).'.png">
            <span class="airtime-company-name ">'.substr($selectdatacard_company_name["company_name"],0,20).'</span>
            <span class="airtime-amount ">'.strtoupper($get_purchased_datacard_pin_fetch["data_size"]).'</span><br>
            ...
            <span class="airtime-serial-no ">Check Data Balance: '.$data_bal.'</span><br>
...
<center>
    <button type="button" id="print-btn" class="button-box color-8 bg-4 mobile-font-size-15 system-font-size-16 mobile-width-50 system-width-30" onclick="printPage();">Print Data Card</button>
</center>
<?php }else{ ?>
...
<center>
	<span class="mobile-font-size-20 system-font-size-25">Strictly for Data Card Printing Transaction Receipt Only!</b></span><br><br>
</center>
</div>
<?php } ?>
