<?php session_start();
	...
	$get_mtn_sme_data_network_qty_price = mysqli_query($conn_server_db, "SELECT sme_data_qty, sme_data_price_1, sme_data_price_2, sme_data_price_3, sme_data_price_4 FROM mtn_sme_data_network_qty_price");
	$get_airtel_sme_data_network_qty_price = mysqli_query($conn_server_db, "SELECT sme_data_qty, sme_data_price_1, sme_data_price_2, sme_data_price_3, sme_data_price_4 FROM airtel_sme_data_network_qty_price");
	$get_9mobile_sme_data_network_qty_price = mysqli_query($conn_server_db, "SELECT sme_data_qty, sme_data_price_1, sme_data_price_2, sme_data_price_3, sme_data_price_4 FROM etisalat_sme_data_network_qty_price");
	...
	if(isset($_POST["buy"])){
		...
		if($carrier == "glo"){
			...
			$apikey = $get_glo_sme_data_apikey["apikey"];
		}
		...
		if($all_user_details["wallet_balance"] >= $amount){
			...
			$log_data_message = "Insufficient Fund, Fund Wallet And Try Again! ";
		}
		
		$_SESSION["transaction_text"] = $log_data_message;
...
<center>
	<div class="container-box bg-4 mobile-width-85 system-width-90 mobile-margin-top-2 system-margin-top-2 mobile-padding-top-3 system-padding-top-3 mobile-padding-left-3 system-padding-left-3 mobile-padding-right-3 system-padding-right-3 mobile-padding-bottom-3 system-padding-bottom-3">
		<form method="post">
			...
			<span style="font-weight: bolder;" class="color-8 mobile-font-size-20 system-font-size-25">BUY SME DATA</span><br>
			...
			<select name="carrier" onchange="updateCarrierAPIkey()" id="carrier-name" hidden>
				...
				<?php if($get_mtn_sme_data_network_status["network_status"] == "active"){ ?>
				<option value="mtn">MTN</option>
				<?php } ?>
				<?php if($get_airtel_sme_data_network_status["network_status"] == "active"){ ?>
				<option value="airtel">Airtel</option>
				<?php } ?>
				<?php if($get_9mobile_sme_data_network_status["network_status"] == "active"){ ?>
			...
			<?php
				...
				if(mysqli_num_rows($get_mtn_sme_data_network_qty_price) > 0){
					while($get_mtn_sme = mysqli_fetch_assoc($get_mtn_sme_data_network_qty_price)){
					if($all_user_details["account_type"] == "smart_earner"){
						$sme_data_price = $get_mtn_sme["sme_data_price_1"];
					}
					
					if($all_user_details["account_type"] == "vip_earner"){
						$sme_data_price = $get_mtn_sme["sme_data_price_2"];
					}
					
					if($all_user_details["account_type"] == "vip_vendor"){
						$sme_data_price = $get_mtn_sme["sme_data_price_3"];
					}
					
					if($all_user_details["account_type"] == "api_earner"){
						$sme_data_price = $get_mtn_sme["sme_data_price_4"];
					}
					
					$sme_data_qty = $get_mtn_sme["sme_data_qty"];
						echo "<option value='".$sme_data_qty.":".$sme_data_price."' >".$sme_data_qty." @ N".$sme_data_price."</option>";
					}
				}
			?>
			</select>
			...
			<?php
				...
				if(mysqli_num_rows($get_airtel_sme_data_network_qty_price) > 0){
					while($get_airtel_sme = mysqli_fetch_assoc($get_airtel_sme_data_network_qty_price)){
					if($all_user_details["account_type"] == "smart_earner"){
						$sme_data_price = $get_airtel_sme["sme_data_price_1"];
					}
					
					if($all_user_details["account_type"] == "vip_earner"){
						$sme_data_price = $get_airtel_sme["sme_data_price_2"];
					}
					
					if($all_user_details["account_type"] == "vip_vendor"){
						$sme_data_price = $get_airtel_sme["sme_data_price_3"];
					}
					
					if($all_user_details["account_type"] == "api_earner"){
						$sme_data_price = $get_airtel_sme["sme_data_price_4"];
					}
					
					$sme_data_qty = $get_airtel_sme["sme_data_qty"];
						echo "<option value='".$sme_data_qty.":".$sme_data_price."' >".$sme_data_qty." @ N".$sme_data_price."</option>";
					}
				}
			?>
			...
			<?php
				...
				if(mysqli_num_rows($get_9mobile_sme_data_network_qty_price) > 0){
					while($get_9mobile_sme = mysqli_fetch_assoc($get_9mobile_sme_data_network_qty_price)){
					if($all_user_details["account_type"] == "smart_earner"){
						$sme_data_price = $get_9mobile_sme["sme_data_price_1"];
					}
					
					if($all_user_details["account_type"] == "vip_earner"){
						$sme_data_price = $get_9mobile_sme["sme_data_price_2"];
					}
					
					if($all_user_details["account_type"] == "vip_vendor"){
						$sme_data_price = $get_9mobile_sme["sme_data_price_3"];
					}
					
					if($all_user_details["account_type"] == "api_earner"){
						$sme_data_price = $get_9mobile_sme["sme_data_price_4"];
					}
					
					$sme_data_qty = $get_9mobile_sme["sme_data_qty"];
						echo "<option value='".$sme_data_qty.":".$sme_data_price."' >".$sme_data_qty." @ N".$sme_data_price."</option>";
					}
				}
			?>
			...
			<script>
				function carrierServiceName(serviceName,netName){
					...
					let servNetArray = ['mtnServNetImg','airtelServNetImg','9mobileServNetImg'];
