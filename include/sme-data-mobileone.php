...
if ($daily_transaction_counter["status"] == true) {
	...
	if (mysqli_num_rows($check_blocked_number_before_recharge) === 0) {
		if ($carrier == "mtn") {
			$data_qty_code = "daily_money_mtn_sme_" . $data_qty . "_30days";
		}
		...
		if ($carrier == "9mobile") {
			$data_qty_code = "9mobile_sme_" . str_replace(".", "_", $data_qty);
		}

		$wallet_balance = $all_user_details["wallet_balance"];
		if ($wallet_balance >= $amount) {
			$dataPurchase = curl_init();
			$dataApiUrl = "https://mobileone.ng/api/v2/datashare/?api_key=" . $apikey . "&product_code=" . $data_qty_code . "&phone=" . $phone_number;
			...
			if ($GetdataJSON == true) {
				if (in_array($dataJSONObj["error_code"], array(1986, 1981))) {
					$log_data_message = $dataJSONObj["data"]["server_message"];
				...
				if (!in_array($dataJSONObj["error_code"], array(1986, 1981))) {
					$log_data_message = "Error: " . $dataJSONObj["server_message"];
				}
