...
if ($daily_transaction_counter["status"] == true) {
	...
	if (mysqli_num_rows($check_blocked_number_before_recharge) === 0) {
		...
		if ($wallet_balance >= $amount) {
			...
			$airtimeApiUrl = "https://grecians.ng/api/v2/airtime/?api_key=" . $apikey . "&product_code=" . $carrier_id . "&phone=" . $phone_number . "&amount=" . $amount;
			...
			1976 MINIMUM ORDER ERROR, INCREASE YOUR ORDER PRICE
			*/
			...
			if ($GetAirtimeJSON == true) {
				...
				if (in_array($AirtimeJSONObj["error_code"], array(1986, 1981))) {
					...
					$checkout_amount = $amount - ($amount * $airtime_discount / 100);
