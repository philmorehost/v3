...
if ($daily_transaction_counter["status"] == true) {
	...
	if (mysqli_num_rows($check_blocked_number_before_recharge) === 0) {
		if ($carrier == "mtn") {
			$net_id = 1;
			$data_gifting_array_prices = array("500mb" => "19", "1gb" => "20", "2gb" => "21", "3gb" => "22", "5gb" => "23", "10gb" => "24");
		}

		if ($carrier == "airtel") {
			$net_id = 2;
			$data_gifting_array_prices = array("500mb" => "7", "1gb" => "8", "2gb" => "9", "5gb" => "10", "10gb" => "26");
		}

		if ($carrier == "glo") {
			$net_id = 3;
			$data_gifting_array_prices = array("500mb" => "30", "1gb" => "31", "2gb" => "32", "3gb" => "33", "5gb" => "34", "10gb" => "35");
		}

		if ($carrier == "9mobile") {
			$net_id = 4;
			$data_gifting_array_prices = array("500mb" => "46", "1gb" => "47", "2gb" => "48", "3gb" => "49", "5gb" => "51", "10gb" => "52");
		}
		...
		if (!empty($data_gifting_array_prices[$data_qty])) {
			...
			if ($wallet_balance >= $amount) {
				...
				if ($GetdataJSON == true) {
					...
					if (!in_array($dataJSONObj["status"], array("success"))) {
						$log_gifting_data_message = "Error: " . $dataJSONObj["success"];
					}

				} else {
					$log_gifting_data_message = curl_error($dataPurchase);
				}
			} else {
