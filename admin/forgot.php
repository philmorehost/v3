<?php session_start();
	...
	if(isset($_POST["recover-pin"])){
		...
		//GET USER DETAILS
		$admin_session = mysqli_real_escape_string($conn_server_db,strip_tags($_POST["rec-email"]));
		$all_admin_details = mysqli_fetch_assoc(mysqli_query($conn_server_db,"SELECT fullname, email, password FROM admin WHERE email='$admin_session'"));
		
		$recovery_code = mysqli_real_escape_string($conn_server_db,strip_tags($_POST["admin-rec-code"]));
		$rec_new_pin = md5(strip_tags($_POST["rec-new-pin"]));
		$raw_number = "1234567890";
		$random_num = substr(str_shuffle($raw_number),0,6);
		$to = $all_admin_details["email"];
		
		if($all_admin_details["email"] == true){
		if(isset($_SESSION["admin-rec-code"])){
			if($_SESSION["admin-rec-code"] == $recovery_code){
				if(mysqli_query($conn_server_db,"UPDATE admin SET password='$rec_new_pin' WHERE email='$admin_session'") == true){
					header("Location: /admin/login.php");
					$log_pin_message = "Admin Password Changed Successfully, You will be redirected to login page shortly! ";
					unset($_SESSION["admin-rec-code"]);
			...
			<span id="content">Hi '.ucwords($all_admin_details["fullname"]).',<br>
				Your Admin Account Password Reset code is: <b>'.$random_num.'</b>
			</span><br>
			<center>
			...
			$_SESSION["admin-rec-code"] = $random_num;
			$_SESSION["rec-email"] = $to;
			$log_pin_message = "Recovery Code Sent Successfully";
		}
		
		}else{
			$log_pin_message = "Email Doesn't Exists";
		}
...
?>
<a style="text-decoration: none;" href="/admin/login.php"><img style="float: left;" class="" src="/images/back-arrow.svg" /></a><br>
<img class="mobile-width-25 system-width-25 mobile-margin-bottom-2 system-margin-bottom-2" src="/images/logo.png" /><br>

<form method="post">
	<span style="font-weight: bolder;" class="color-5 mobile-font-size-20 system-font-size-25">Reset Admin Password</span><br><br>
	...
	<input name="rec-email" pattern" type="email" class="input-box mobile-width-95 system-width-95 mobile-margin-top-3 system-margin-top-3 mobile-margin-bottom-5 system-margin-bottom-5" placeholder="abc@gmail.com" <?php if(isset($_SESSION["rec-email"])){ echo "readonly"; } ?> value="<?php echo $_SESSION['rec-email']; ?>"/><br>
	<?php if(isset($_SESSION["admin-rec-code"])){ ?>
		...
		<input name="admin-rec-code" pattern="[0-9]{6}" title="Code must be Number and not more/less than 6 digit" type="text" class="input-box mobile-width-95 system-width-95 mobile-margin-top-3 system-margin-top-3 mobile-margin-bottom-5 system-margin-bottom-5" placeholder="123456" required/><br>
		...
		<input name="rec-new-pin" type="text" class="input-box mobile-width-95 system-width-95 mobile-margin-top-3 system-margin-top-3 mobile-margin-bottom-5 system-margin-bottom-5" placeholder="••••••••" required/><br>
	<?php } ?>
	...
	<span class="color-3 mobile-font-size-12 system-font-size-15">Suddenly Remember Password? <a style="text-decoration: none; font-size:inherit; font-weight:lighter;" class="color-5" href="/admin/login.php">Sign In</a></span><br>
</form>
<form method="post">
	<?php if(isset($_SESSION["admin-rec-code"])){ ?>
