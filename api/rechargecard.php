<?php
	...
	if(!empty($api_token) && !empty($api_network) && !empty($api_qty) && !empty($api_amount)){
		...
		if(mysqli_num_rows($token_owner_details) == 1){
			if(in_array($api_network,array("mtn","airtel","glo","9mobile"))){
				if(in_array($api_amount,array(100,200,500))){
					...
					//GET EACH rechargecard API WEBSITE
					...
					$network_state = $get_mtn_rechargecard_network_status["network_status"];
					}
					...
					$network_state = $get_airtel_rechargecard_network_status["network_status"];
					}
					...
					$network_state = $get_glo_rechargecard_network_status["network_status"];
					}
					...
					$network_state = $get_9mobile_rechargecard_network_status["network_status"];
					}
					
					$discounted_price_amount = (($amount-($amount*($rechargecard_discount/100)))*$qty);
					...
					$message = array("code"=>400,"desc"=>"Amount must be 100, 200 or 500");
					echo json_encode($message,true);
				}
			}else{
