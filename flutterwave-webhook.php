<?php
	...
	$flutterwave_verify_transaction = json_decode(getAPIBalance("GET","https://api.flutterwave.com/v3/transactions/".$catch_incoming_request["data"]["id"]."/verify",["Authorization: Bearer ".$flutterwave_keys["secret_key"]],""),true);
	...
	if(($flutterwave_verify_transaction["status"] == "success") && ($catch_incoming_request["data"]["status"] == "successful")){
		...
		$insert_transaction_data = "INSERT INTO transaction_history (email, id, amount, w_bef, w_aft, status, description, transaction_type, website) VALUES ('$customer_email','$transaction_id','$charged_amount', '".$get_current_balance["wallet_balance"]."', '$new_balance', 'successful','Wallet Funding','wallet-funding','flutterwave.com')";
